...
export const HarmonicsTuningAdvisor: React.FC<HarmonicsTuningAdvisorProps> = ({
  ...
  const recommendedCli = `# Rotorflight 2.x Recommended Filter & Notch Settings
# Generated by Rotorflight Blackbox Vibration Analyzer
# Target Headspeed: ${Math.round(mainRpm)} RPM

set dyn_notch_count = 3
set dyn_notch_q = 350
set dyn_notch_min_hz = ${Math.max(20, Math.round(main1P * 0.85))}
set dyn_notch_max_hz = ${Math.round(tail1P * 1.6)}

# Harmonic Notch (RPM Tracking)
# 1P Main Rotor Filter (~${Math.round(main1P)} Hz)
set gyro_harm_notch_harmonics = 3
set gyro_harm_notch_min_hz = ${Math.round(main1P * 0.8)}

# Tail Rotor RPM tracking (~${Math.round(tail1P)} Hz)
# Set tail gear ratio in Rotorflight governor config
set tail_gear_ratio = ${Math.round(config.tailGearRatio * 100)}
