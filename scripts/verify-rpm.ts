...
// Case 6: screenshot case — weak 1P (47.3Hz) + strong 2P (not 79.5Hz but 94.6Hz)
// In reality the dominant peak is not 1P=47.3 but 2P=79.5 -> the real 1P is 39.75Hz = 2385RPM
// Synthetic: 1P 39.75Hz (amplitude 2) + 2P 79.5Hz (amplitude 10)
{
  const sr6 = 2000;
  const n6 = sr6 * 10;
  ...
  // The old method (max peak) wrongly gives 79.5*60=4770; the new method must give the correct 39.75*60=2385
  const ok6 = Math.abs(a6 - 2385) <= 100;
  console.log(`[2P-dominant] avg=${a6.toFixed(1)} expected=2385 ${ok6 ? 'PASS' : 'FAIL (2P misdetection)'} valid=${v6.length}/${s6.rpm.length}`);
}
